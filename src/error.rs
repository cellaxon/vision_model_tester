        ...
        C: std::fmt::Display,
    {
        self.map_err(|e| {
            ...
            match app_error {
                ...
                AppError::InferenceError(msg) => AppError::InferenceError(format!("{}: {}", context, msg)),
...
pub mod validation {
    ...
    pub fn validate_confidence_threshold(threshold: f32) -> AppResult<()> {
        if !(0.0..=1.0).contains(&threshold) {
            return Err(AppError::ValidationError(
                format!("신뢰도 임계값은 0.0과 1.0 사이여야 합니다: {}", threshold)
            ));
        }
        Ok(())
    }
    ...
    pub fn validate_nms_threshold(threshold: f32) -> AppResult<()> {
        if !(0.0..=1.0).contains(&threshold) {
            return Err(AppError::ValidationError(
                format!("NMS 임계값은 0.0과 1.0 사이여야 합니다: {}", threshold)
            ));
        }
        Ok(())
    }
