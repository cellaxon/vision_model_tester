...
pub mod fs_utils {
    ...
    pub fn get_home_directory() -> AppResult<String> {
        env::var("HOME")
            ...
            .or_else(|_| env::current_dir().map(|dir| dir.to_string_lossy().to_string()))
            .map_err(|_| {
                crate::error::AppError::ConfigError("홈 디렉토리를 찾을 수 없습니다".to_string())
            })
    }
    ...
    pub fn get_app_data_directory() -> AppResult<std::path::PathBuf> {
        ...
        // 디렉토리가 없으면 생성
        if !app_dir.exists() {
            std::fs::create_dir_all(&app_dir).map_err(crate::error::AppError::IoError)?;
        }
